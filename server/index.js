...
// CONFIGURATIONS (ALL MIDDLEWARE AS WELL AS PACKAGE CONFIGURATIONS) // Middleware means function that runs between different functions.
const __filename = fileURLToPath(import.meta.url); // When you use `type:module` then it's reqired. Used to grab URL.
...
app.use(cors());
app.use("/assets", express.static(path.join(__dirname, "public/assets"))); // It's setting a directory where we keep our assets // In real project we need to stored it in cloud storage.
...
const upload = multer({storage}); // Whenever we want to upload a file we will use `upload` variable.
